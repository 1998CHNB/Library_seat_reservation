...
def get():
    ...
    dt=datetime.now()
    sleeptime = int(yuyueday)*24*3600+(yuyuehour-dt.hour)*3600+(yuyueminute-dt.minute)*60-dt.second
    t = Timer(sleeptime, reservation)
    t.start()
window = tk.Tk()
window.title('图书馆座位预约')
window.geometry('1000x700')
l = tk.Label(window, text='你好，为保证预约成功，请提前登陆微信，并最好置顶公众号：“我去图书馆” 。', bg='blue', font=('Arial', 12), width=60, height=2)
l.pack()
tishi1 = tk.Label(window, text='请先输入你想要几天后预约:', font=('Arial', 12), width=30, height=1)
tishi1.pack()
e1 = tk.Entry(window, show = None)
e1.pack()
tishi2 = tk.Label(window, text='请输入你想要预约当天的具体时间:(格式-例如：6:01请输入：06:01)', font=('Arial', 12), width=55, height=1)
tishi2.pack()
...
l1.pack()
l2 = tk.Label(window, text='初次使用时最好将文件夹下的png图片换成自己电脑上对应的截图',font=('Arial', 12), width=50, height=1)
l2.pack()
l3 = tk.Label(window, text='这样能使图像识别的准确率提升；图片和exe文件需放在同一个文件夹下',font=('Arial', 12), width=55, height=1)
l3.pack()
def reservation():
    T = 0
    while T < 2:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    pyautogui.click(clicks=1)
                    time.sleep(3)
                    pyautogui.moveTo(cc[0]+int(pingmu_X/13),cc[1]+int(pingmu_Y/1.9))  
                    pyautogui.click(clicks=1)
                    time.sleep(3)
                except:
